...
namespace Subfinder
{
	public class Subfinder
	{
		...
		static void Main(String[] args)
		{
			...
			var client = new Subfinder(args);
			client.Run ();

			Application.Run ();
		}

		public Subfinder (String[] args)
		{
		...
		void ConfigureTreeView ()
		{
			...
			var configureColumn = 
				new System.Action<string, CellRenderer, int, object[]>((columnName, cellRenderer, columnNo, p) => {
			...
			configureColumn ("Download?", rendererToggle, 0, new object[]{"active", 0});
			configureColumn ("Rating", new CellRendererText (), 1, new object[]{ "text", 1 });
			configureColumn ("Downloads count", new CellRendererText (), 2, new object[]{ "text", 2 });
			configureColumn ("Language", new CellRendererText (), 3, new object[]{ "text", 3 });
			configureColumn ("Subtitles database", new CellRendererText (), 4, new object[]{ "text", 4 });
		...
		{
			var videoChooser = new FileChooserDialog ("Choose the file to open",
		...
		void SearchBtnClick (object sender, EventArgs e)
		{
			if (!File.Exists (videoFileName.Text))
				throw new Exception ("File " + videoFileName.Text + "doesn't exists");
			...
			if (!enumerable.Any()) 
			{
				ShowMessage ("Subtitles not found");
			}
		}
		...
		void DownloadSelectedBtnClick (object sender, EventArgs e)
		{
			...
			downloader.DownloadCompleted += (sdr, evt) => Application.Invoke ((sndr, evnt) => ShowMessage ("Download completed!"));
		}
